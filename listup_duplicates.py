...
class Glyph(object):
	...
	def getBuhin(self):
		...
		for row in self.data:
			...
			if not g or self is g or g in buhin_stack:
				b = []
				logging.error("'{}' was not found or has a quotation loop".format(bn))
				break
			...
			buhin_stack[-1:] = []
			if bbs:
				scale = [(area[2] - area[0]) / 200.0, (area[3] - area[1]) / 200.0]
				for bb in bbs:
					b.append((
						area[0] + bb[0] * scale[0],
						area[1] + bb[1] * scale[1],
	...
	def getBuhinHash(self):
		if len(self.data) == 1 and self.data[0][0:19] == "99:0:0:0:0:200:200:":
			return ()
		if "_" in self.name:
			return ()
		return tuple(b[4] for b in self.getBuhin())
	...
	def getKaku(self):
		...
		for row in self.data:
			...
			if r[0] == "1":
				...
				k.append((1, int(dir1 + 7 * (r[1] // 10) + 28 * (r[2] * 2 if 0.0 < r[2] < 10.0 else r[2] // 10)), r[3], r[4], r[5], r[6]))
			elif r[0] == "2":
				r = [float(x) for x in r]
				if r[1] == 32.0 and r[2] == 0.0:
					if (r[4] == r[8] and r[3] < r[7]) or r[4] > r[8]:
						r[3:9] = r[7:9] + r[5:7] + r[3:5]
				if r[2] == 0.0 and r[1] in (0.0, 12.0, 22.0, 32.0) and 0 != abs(r[4] - r[8]) > r[7] - r[3] and abs(
					r[3] + (r[7] - r[3]) * (r[6] - r[4]) / (r[8] - r[4]) - r[5] if abs(r[4] - r[8]) > abs(r[3] - r[7]) else
					r[4] + (r[8] - r[4]) * (r[5] - r[3]) / (r[7] - r[3]) - r[6]
				) <= 5.0:
					dir1 = cmp(r[3], r[7]) * 2 + cmp(r[4], r[8]) + 3
					k.append((1, int(dir1 + 7 * (r[1] // 10) + 84), r[3], r[4], r[7], r[8]))
					continue
				dir1 = cmp(r[3], r[5]) * 2 + cmp(r[4], r[6]) + 3
				dir2 = cmp(r[5], r[7]) * 2 + cmp(r[6], r[8]) + 3
				k.append((2, int(dir1 + 7 * dir2 + 49 * (r[1] // 5) + 392 * (r[2] // 5)), r[3], r[4], r[5], r[6], r[7], r[8]))
			elif r[0] == "6" or r[0] == "7":
				r = [float(x) for x in r]
				if r[1] == 32.0 and r[2] == 0.0:
				...
				if r[2] == 0.0 and r[1] in (0.0, 12.0, 22.0, 32.0) and 0 != abs(r[4] - r[10]) > r[9] - r[3] and max(
					(
						abs(r[3] + (r[9] - r[3]) * (r[6] - r[4]) / (r[10] - r[4]) - r[5]),
						abs(r[3] + (r[9] - r[3]) * (r[8] - r[4]) / (r[10] - r[4]) - r[7])
					) if abs(r[4] - r[10]) > abs(r[3] - r[9]) else (
						abs(r[4] + (r[10] - r[4]) * (r[5] - r[3]) / (r[9] - r[3]) - r[6]),
						abs(r[4] + (r[10] - r[4]) * (r[7] - r[3]) / (r[9] - r[3]) - r[8])
					)
				) <= 5.0:
					dir1 = cmp(r[3], r[9]) * 2 + cmp(r[4], r[10]) + 3
					k.append((1, int(dir1 + 7 * (r[1] // 10) + 84), r[3], r[4], r[9], r[10]))
					continue
				dir1 = cmp(r[3], r[5]) * 2 + cmp(r[4], r[6]) + 3
				dir2 = cmp(r[7], r[9]) * 2 + cmp(r[8], r[10]) + 3
				k.append((2, int(dir1 + 7 * dir2 + 49 * (r[1] // 5) + 392 * (r[2] // 5)), r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10]))
			elif r[0] == "3" or r[0] == "4":
				r = [float(x) for x in r]
				k.append((3, int((r[1] // 10) + 4 * (r[2] // 5)), r[3], r[4], r[5], r[6], r[7], r[8]))
			elif r[0] == "99":
				bn = r[7].split("@")[0]
				g = dbn.get(bn)
				if not g or self is g or g in buhin_stack:
				...
				for bkaku in bk:
					...
					if isStretched:
						for i in range(0, l, 2):
							points[i] = stretch(params[0], params[2], points[i])
							points[i + 1] = stretch(params[1], params[3], points[i + 1])
					for i in range(0, l, 2):
	...
	def getKakuHash(self):
		if len(self.data) == 1 and self.data[0][0:19] == "99:0:0:0:0:200:200:":
			return ()
		if "_" in self.name:
			return ()
		return tuple(k[0:2] for k in self.getKaku())
	xorMaskType = 0
...
class Response(object):
	def __init__(self, f):
		...
		self.header_msg = f.headers
		self.final_url = f.url
		f.close()


def fetch(url, payload=None, method=GET, headers={}, deadline=5):
	req = urllib2.Request(url, headers=headers)
	if method == POST:
		req.add_data(payload)
	try:
		f = urllib2.urlopen(req, timeout=deadline)
		res = Response(f)
...
neg_data = fetch(neg_url, deadline=60).content
...
del re, neg_url, neg_data, neg_src, neg_masktype
...
for glyph in db:
	try:
		bh = glyph.getBuhinHash()
		if bh:
			buhin.setdefault(bh, []).append(glyph)
		kh = glyph.getKakuHash()
		if kh:
