...
class Glyph(NamedTuple):
    ...
    def isAlias(self):
        return len(self.data) == 1 and self.data[0].startswith("99:0:0:0:0:200:200:")
...
def stretch(dp: float, sp: float, p: float, pmin: float = 12.0, pmax: float = 188.0):
    if p < sp + 100.0:
        p1 = pmin
        p3 = pmin
        p2 = sp + 100.0
...
class BuhinSimilarGlyphFinder(
    ...
    def is_similar_summary(cls, summary1: BuhinSummary, summary2: BuhinSummary) -> bool:
        for b1, b2 in zip(summary1, summary2):
            ...
            b2p1, b2p2 = b2.coords
            if (
                abs((b1p1[0] + b1p2[0]) - (b2p1[0] + b2p2[0])) <= 20.0
                and abs((b1p1[1] + b1p2[1]) - (b2p1[1] + b2p2[1])) <= 20.0
            ):
                continue
            return False
        return True
...
def dist_from_line(x0: float, y0: float, x1: float, y1: float, x: float, y: float):
...
def get_kaku_info(
    ...
    if stype == 1:
        ...
        if (
            endType == 0
            and sttType in (0, 12, 22, 32)
            and 0 != abs(y0 - y2) >= x2 - x0
            and is_almost_straight(coords)
        ):
...
class KakuSimilarGlyphFinder(
    GlyphSummaryManagerMixin[KakuSummary], SimilarGlyphFinderBase[KakuSummary, KakuHash]
):
    def _get_summary_impl(self, name: str) -> KakuSummary:
        ...
        for row in self.dump[name].data:
            ...
            if line_data[0] != "99":
                ...
                kaku_sig = KakuElem(kaku_type, dirs + shapes, (points[0], points[-1]))
                k.append(kaku_sig)
                continue
            ...
            if not dpx == dpy == 0.0:
                ...
                stretch_x = stretch_mapper(
                    dpx, spx, [x for b_kaku in b_kakus for (x, y) in b_kaku.coords]
                )
                stretch_y = stretch_mapper(
                    dpy, spy, [y for b_kaku in b_kakus for (x, y) in b_kaku.coords]
                )
...
def getDump(path: Path) -> Dump:
    ...
    with path.open(encoding="utf-8") as dumpfile:
        ...
        for line in dumpfile:
            ...
            if rel == "u3013":
                rel = None
            glyph = Glyph(name, rel, gdata.split("$"), masktype_map.get(name, 0))
            dump[name] = glyph
    return dump
